...
async def sync_command(bot:commands.Bot, guild_ids:[]):
    @bot.tree.command(name ="add_week", description="Pushes new semester material into the bot.", guilds=[discord.Object(id=gid) for gid in guild_ids]) #Add the guild ids in which the slash command will appear. If it should be in all, remove the argument, but note that it will take some time (up to an hour) to register the command if it's for all guilds.
